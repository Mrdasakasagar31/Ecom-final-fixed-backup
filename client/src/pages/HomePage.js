...
const HomePage = () => {
    ...
    // Fetch categories, products, and total count
    const getAllCategory = async () => {
        try {
            const { data } = await axios.get("https://ecom-final-fixed-backup.onrender.com/api/v1/category/get-category");
            if (data?.success) {
            ...
            <div className="container-fluid row mx-auto">
                ...
                <div className="col-md-9 col-sm-12">
                    ...
                    <div style={{ position: 'relative', minHeight: '50vh' }}>
                        ...
                        <div className={`d-flex flex-wrap justify-content-center ${loading ? 'opacity-50' : ''}`}>
                            {products?.map((p) => (
                                <div key={p._id} className="card m-3" style={{ width: "18rem", height: 'auto', minHeight: '20rem' }}>
                                    <img
                                        src={`https://ecom-final-fixed-backup.onrender.com/api/v1/product/product-photo/${p._id}`}
                                        className="card-img-top"
                                        alt={p.name}
                                    ...
                                    <div className="card-body" style={{ backgroundColor: '#f9f9f9', padding: '1rem', borderRadius: "0 0 8px 8px" }}>
                                        ...
                                        <div className='d-flex justify-content-between mt-2'>
                                            ...
                                            <button 
                                                ...
                                                style={{ fontSize: '0.85rem', fontWeight: '500', marginLeft: '10px' }}
                                            >
                                                ADD TO CART
                                            </button>
                                        </div>
                    ...
                    <div>
                        {products && products.length < total && (
                            <div className="text-center mt-5" style={{ marginBottom: '60px' }}>
                                <button 
                                    className='btn btn-dark mb-5 mt-3 mx-auto' 
                                    onClick={(e) => {
                                        e.preventDefault();
                                        setPage(page + 1);
                                    }}
