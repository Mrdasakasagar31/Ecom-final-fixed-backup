...
const CreateCategory = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            const { data } = await axios.post("https://ecom-final-fixed-backup.onrender.com/api/v1/category/create-category", {
                name,
            })
    ...
    const handleUpdate = async (e) => {
        e.preventDefault()
        try {
            const { data } = await axios.put(`https://ecom-final-fixed-backup.onrender.com/api/v1/category/update-category/${selected._id}`, { name: updatedName })
            if (data.success) {
    ...
    const handleDelete = async (pId) => {
        try {
            const { data } = await axios.delete(`https://ecom-final-fixed-backup.onrender.com/api/v1/category/delete-category/${pId}`)
            if (data.success) {
